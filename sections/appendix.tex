...
\begin{table}[htbp]
  \centering\small
  \caption{Frozen configurations per model and regime, selected once on the burn-in and held fixed for every refit block. The deep models additionally run seeds 42/43/44.}
...
\begin{table}[htbp]
  \centering\small
  \caption{Shared hyperparameter-search protocol. Every tuned model runs the same sampler, trial budget and selection window.}
...
\begin{table}[htbp]
  \centering\small
  \caption{Sequence-model hyperparameter search spaces. Rows shared by both architectures are identical by design; ``---'' marks a parameter that does not exist for that architecture.}
...
\begin{table}[htbp]
  \centering\small
  \caption{LightGBM hyperparameter search space, one booster per quantile. \texttt{lag\_set} selects one of six nested lag sets $\{1\},\dots,\{1,2,3,6,12,24\}$.}
...
\begin{table}[htbp]
  \centering\small
  \caption{Frozen neural-network hyperparameters. Depth is xLSTM blocks or LSTM layers, width the embedding or hidden size; ``---'' marks a parameter that does not apply.}
...
\begin{table}[htbp]
  \centering\small
  \caption{Frozen LightGBM hyperparameters per regime, one booster per quantile. \texttt{max\_depth}${}=-1$ means no depth limit, so growth is bounded by \texttt{num\_leaves}.}
...
\begin{table}[htbp]
  \centering\small
  \caption{Frozen ARIMA orders per country, selected by AIC on the differenced target in regime L. France collapses to $(0,0)$, which reproduces the Random Walk.}
...
\begin{table}[t]
\centering
\caption{Per-country breakdown. Gate rate is the share of admissible cells. Tail asymmetry is the share of lower violations. Base rate is $\max(p_{\uparrow},1-p_{\uparrow})$ of realized directions.}
...
\begin{table}[t]
\centering
\caption{Tail asymmetry over admissible combinations, by model family and by method. It is the share of lower violations among all violations, and 0.5 marks symmetry.}
\label{tab:app_tail}
...
\begin{table}[t]
\centering
\caption{Mean coverage in percent by calibration-pool window. ``---'' marks a method that does not read the pool, so the window variants do not apply.}
\label{tab:app_pool}
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[t]
\centering
\caption{Seed dispersion of the neural families on the left, and the median skill of each family's best admissible package with Ireland excluded on the right.}
...
\textit{Notes.} The span is the range of the median Winkler skill across seeds 42/43/44. ``span $>|$skill$|$'' is the share of that family's combinations whose span exceeds the absolute value of its median skill. On the right, Skill is the median over the ten countries against \texttt{rw}/L/\texttt{native}, so a positive value beats that reference. The ex-IE column recomputes it over the nine countries without Ireland. Rows on the right name the model family only, and \cref{tab:res_ranking} gives the full package.
\end{minipage}}
\end{table}
