...
Our reporting protocol is \emph{validity-first}. The interval calibration is
treated as an admissibility \emph{gate}, and the Winkler interval score is
the sole ranking criterion \emph{within} the admissible set. This operational
reading of ``maximise sharpness subject to calibration''
\parencite{gneiting2007sharpness} is made precise in \cref{sec:eval:validity}.
The unit of evaluation, which we call a cell, is a
model\,$\times$\,regime\,$\times$\,seed\,$\times$\,method\,$\times$\,country
tuple. Multiplicity across cells is controlled throughout by
...
\begin{equation}
  ...
  \label{eq:mae_ratio}
\end{equation}
\noindent where $T$ is the total number of periods in the common out-of-sample evaluation period, and $t$ indexes the specific point in time. The actual observed value for country $c$ at time $t$ is given by $s_{t}(c)$. $\hat{s}_{m,t}(c)$ represents the point forecast (median of the predictive distribution) generated by model $m$ and $\hat{s}_{\mathrm{RW},t}(c)$ is the forecast of the naive Random Walk baseline.\\
The ratios are first calculated per country and then the median is taken across all countries. At the raw point forecast our \emph{tirex2} model was practically on par with the Random Walk (MAE-Ratio = 0.9965). This underscores the narrative of our paper. The main advantage of the model is not a superior point forecast but a superiority in predicting intervals and correctly quantifying uncertainty \parencite{hyndman2006mase}. We also measure the directional forecast quality, i.e.\ if the model gets the trend right. For that we employ the Pesaran--Timmermann test \parencite{pesaran1992} in \eqref{eq:pt_test} on the sign of the median $\Delta$ forecast.
\begin{equation}
  ...
  \label{eq:pt_test}
\end{equation}
\noindent In this test statistic, $\hat{P}_S$ is the observed proportion of correctly predicted directional changes (the hit rate) and $\hat{P}_*$ the expected proportion of correct predictions under the null hypothesis of directional independence (i.e., pure chance). The terms $\widehat{\operatorname{Var}}(\hat{P}_S)$ and $\widehat{\operatorname{Var}}(\hat{P}_*)$ denote the consistent estimates of the respective variances. Under the null hypothesis, the test statistic asymptotically converges in distribution to a standard normal distribution ($\mathcal{N}(0,1)$).
