...
% SCHREIBE: Kapitelkopf Evaluation (4-6 Sätze).
%
%  1. WO gemessen wird (1 Satz):
%     - Alle Intervall- und Punktmetriken im LEVEL-Raum.
%     - Auf dem gemeinsamen Testfenster: 127 Monate, 2015-10 bis 2026-04.
%     - Erst nachdem die Delta-Prognosen auf Levels zurückverankert
%       wurden. Querverweis -> \cref{sec:data:target} (NICHT sec:exp:target,
%       das gibt es nicht).
%     - Warum Level und nicht Delta: die Intervalle, die jemanden
%       interessieren, leben im Level-Raum. Ein Satz dazu, sonst wirkt es
%       wie eine technische Fußnote.
%
We measure everything in level space, only after the $\Delta$ forcast ist transformed back into level space, on the shared test window of 127 months from 2015-10 to 2026-04 as can be seen in \cref{sec:data:target}. We evaluate in the level space since the intervalls that we are interested in live in the level space. \\
A Model is either passes the calibration Gate and is admissible or not. We rank only inside the admissible set by winkler score and not based on the calibration gate. More detailed inforamtion in \cref{sec:eval:validity}. 

%  2. DAS TRAGENDE PRINZIP (2 Sätze) — das ist der Kern des Absatzes:
%     - Kalibrierung ist ein GATE, kein Rangkriterium. Eine Kombination
%       ist entweder zulässig oder nicht.
%     - Innerhalb der zulässigen Menge entscheidet allein der
%       Winkler-Score über den Rang.
%     - Das ist deine operative Lesart von "sharpness subject to
%       calibration" \parencite{gneiting2007sharpness}.
%     - PFLICHT: Verweis auf \cref{sec:eval:validity}, wo du es präzise
%       machst.
%     -> Die Original-Wendung "Our reporting follows a validity-first
%        logic" ist der Tell. Sag dieselbe Sache, aber sag WARUM du es so
%        machst: ein Intervall, das die Coverage verfehlt, ist nicht
%        "etwas schlechter", sondern schlicht ungültig. Deshalb Gate.
We evaluate per cell, which consists of Modell x Regime x Seed x Method x Country. Multiplicity over all cells is corrected for via Benjamini-Hochberg-FDR \parencite{benjamini1995fdr}.
%  3. DIE AUSWERTUNGSEINHEIT (1-2 Sätze):
%     - Eine "cell" = Modell x Regime x Seed x Methode x Land.
%     - Multiplizität über alle Zellen wird durchgehend mit
%       Benjamini-Hochberg-FDR kontrolliert \parencite{benjamini1995fdr}.
%
%  NICHT hier: Zahlen aus den Ergebnissen. Keine Skills, keine
%  Coverage-Werte. Das ist das Results-Kapitel.
%  NICHT hier: die 70%-Schwelle und q=0.10 im Detail -> sec:eval:validity.
...
\begin{equation}
  ...
  \label{eq:skill}
\end{equation}
We use the Mean Absolute Error (MAE) and Root Mean Square Error (RMSE) ratios of the model's median forecast relative to the native Random Walk baseline to evaluate the point forecast performance. 
\begin{equation}
...
% SCHREIBE: Teil 1 — Symbolerklärung MAE-Ratio (2-3 Sätze).
%  Ein Formelerklärer muss sein, der Leser braucht die Symbole. Der Tell
%  ist nicht, DASS du erklärst, sondern die Aufzählungskadenz
%  ("X is ..., Y is ..., and Z is ..."). Brich sie auf: zwei kurze Sätze
%  statt einer Kette, oder ein Satz plus Halbsatz.
%   - T = Zahl der Monate im gemeinsamen Out-of-Sample-Zeitraum, t der
%     Zeitindex.
%   - s_t(c) = beobachteter Wert für Land c zur Zeit t.
%   - s^_{m,t}(c) = Punktprognose von Modell m; PFLICHT dazuzusagen, dass
%     das der Median der prädiktiven Verteilung ist, nicht der Mittelwert.
%   - s^_{RW,t}(c) = Prognose des Random Walk als Referenz.

\noindent Here T is the number of months in the shared out of sample period and t the time index. $s_t(c)$ is the observed value for country c at time t while $\bar{s}_{m,t}(c)$ ist the point estimator of model m, so the 0.5 quantile. $\bar{s}_{RW,t}(c)$ is the point forecast of the random walk reference.

%   - Die Ratios werden erst je Land berechnet, dann wird der Median über
%     die Länder genommen. (Gleiche Logik wie in \cref{sec:exp:seeds} —
%     ein Rückverweis spart dir hier die Wiederholung der Begründung.)
%   - ZAHL: tirex2 liegt beim rohen Punktforecast praktisch gleichauf mit
%     dem Random Walk, MAE-Ratio = 0.9965. Gegen cells.csv prüfen.
%   - Die Einordnung: der Vorteil des Modells liegt NICHT im
%     Punktforecast, sondern in der Intervallprognose und der
%     Unsicherheitsquantifizierung \parencite{hyndman2006mase}.
%     -> Das ist deine These, und du sagst sie hier zum ersten Mal in
%        Zahlen. Formulier sie als deine Schlussfolgerung, nicht als
%        Beobachtung, die sich selbst zieht. Die alte Wendung "This
%        matches the narrative of our paper" ist ein Rest aus dem
%        Humanize-Lauf (davor stand "This underscores the narrative").
%   - Überleitung: zusätzlich wird die Richtungsqualität gemessen, also
%     ob das Modell den Trend trifft. Dafür der Pesaran-Timmermann-Test
%     \parencite{pesaran1992} auf dem Vorzeichen der Median-Delta-
%     Prognose, \eqref{eq:pt_test}.

The ratios are first calculated per country only then the median over all countrys is taken. The same approacha s in \cref{sec:exp:seeds}. Tirex2 and the random walk baseline are basically tied at the point forecast with tirex havin a mea ratio of 0.9965. We conclude that the advantage of a Model is not in the pointforecast but in the uncertainty quantification \parencite{hyndman2006mase}. The pesaran-timmermann test \parencite{pesaran1992} is used on the sign of the median delta forecast to measure the directional forecast quality of the Models, \eqref{eq:pt_test}.
...
\begin{equation}
  ...
  \label{eq:pt_test}
\end{equation}
% SCHREIBE: Teil 3 — Symbolerklärung PT-Test (2-3 Sätze).
%   - P^_S = beobachteter Anteil korrekt vorhergesagter Richtungs-
%     änderungen, also die Trefferquote.
%   - P^_* = der Anteil, den man unter der Nullhypothese der
%     Richtungsunabhängigkeit erwarten würde, also bei reinem Zufall.
%   - Die beiden Var-Terme sind konsistente Schätzer der jeweiligen
%     Varianzen.
%   - Unter der Null ist die Statistik asymptotisch standardnormal.
%   - Letzter Satz, gehört inhaltlich dazu: für alle berechneten Skills
%     gilt dieselbe Referenz, der Random Walk je Land.
%   ACHTUNG: Der Originalabsatz endet mit einem Satz, der nur wiederholt,
%   was die Formel schon sagt ("Under the null the statistic is
%   asymptotically standard normal" steht auch als Pfeil in der Gleichung).
%   Beim Neuschreiben brauchst du ihn nicht zweimal.
\noindent Here $\hat{P}_S$ is the amount of correctly predicted directions while $\hat{P}_*$ is the ratio that would be suspected under the null hypothesis of directional dependency, so in randomness. Both Var therms are consistent estimators of the respective varaince. Under the null hypothesis the statistic is asymptoticaly normal distributed. For all calculated skills we use the same refference: the random walk per country. 
...
%
% ZAHLEN AUS (korrigiert 2026-08-03 -- die frueher hier genannten Dateien
% coverage_diagnostics.parquet / master_summary.parquet /
% coverage_rejection_summary.parquet stammen aus der ABGELOESTEN taeglichen
% v5.3-Pipeline und existieren im aktuellen Lauf nicht mehr. Nicht danach
% suchen, die Zahlen stehen woanders):
%   results/tables/calibration_tests.csv (7420 x 15):
%       model, regime, seed, cp_method, country, n_viol,
%       p_uc, p_ind, p_cc, p_dq, q_uc, q_ind, q_cc, q_dq, cell_valid
%       -> 7420 = 742 Kombinationen x 10 Laender, Primaerfall pool=rolling
%   results/tables/validity_flags.csv (742 x 6):
%       model, regime, seed, cp_method, valid_country_share, validity_flag
%   results/tables/cells.csv -> coverage, cov_se_mbb, cov_first36, cov_rest
%       (Coverage-Ebene je Land x Kombination x pool_mode)
%   Aggregierte Ablehnraten sind NICHT vorberechnet -- sie entstehen als
%   Mittel ueber die q-Spalten von calibration_tests.csv. Die in 8.2
%   berichteten Werte (UC 20.6% / IND 6.1% / CC 26.3% / DQ 49.3% bei
%   q<0.10) sind genau diese Mittel.
%
% ZUSATZ ZU PUNKT 4 (DQ), beim Schreiben mit aufnehmen: der Breiten-
%   Regressor ist kein Beiwerk, sondern der Grund, warum DQ so viel
%   haerter verwirft als IND. Hier nur den Test einfuehren; die
%   Zerlegung und ihre Konsequenz stehen im Limitations-Block L1 am
%   Ende von results.tex. EINEN Vorwaertsverweis setzen, damit der
%   Leser den 49.3%-Wert in 8.2 nicht unerklaert stehen sieht.
%
...
\begin{equation}
  ...
  \label{eq:dq}
\end{equation}
where $\mathrm{Hit}_t=I_t-\alpha$ is the centered violation, $[\hat{l}_t,\hat{u}_t]$
...
%
To compare the packages we did pairwise, per country, Diebold-Mariano contrasts on the winkler differentials $(d_t = W_{A,t} - W_{B,t})$ with Harvey-Leybourne-Newbold Small-Sample-Korrektur for h=1 on the common sample.
$$DM = \frac{\bar{d}}{\sqrt{\frac{\widehat{\text{LRV}}_{\text{HLN}}}{n}}}, \quad d_t = W_{A,t} - W_{B,t}$$Our DM test is a test on the forecast and not on the underlying Model. Because of this the loss of the normal distibution in the DM test for nested Models is not relevant. 10 strongly correlated countrys cant be seen as 10 idependant tests, so, on the panel level, we test on the monthly mean over all countrys. First $\bar{d}_t = \frac{1}{10} \sum_c d_{c,t}$ is calculated, then the t-Test statistic is calculated on the $\bar{d}_t$ with ewey-West-HAC (Lag 3) to correct for the autocorrelation. \\

Our preregisted contrast familys where: RQ3: method vs native, RQ1: G vs L, GF vs G, GF vs G+I, RQ2: model vs const-Zwilling, arma vs $arma\_monthly$. This acts as our defense against p-hacking. 
% WAS HIER STEHT:
...
Post Hoc we did the best valid package per model family contrasts. They have their own FDR family and the tests are overly confident since the the best models where chosen from the same set they are tested on. Due to this fact we can read the test outcomes only as descriptive.
...
We report three descriptive metrics: the differential $d_{\text{norm}} = \frac{\bar{d}}{\bar{W}_{\text{ref}}}$, $share\_a\_better\ (Share\ of\ countrys\ with\ A\ better)$, median-p over countrys. 
...
\label{sec:eval:mcs}
%------------------------------------------------------------------------
The Idea of the Model Confidence Set (MCS) is to find the set of models that, at level $(1 - alpha_{MCS}=0.10)$, is not statisticaly distinguishable from the best Model \citep{hansen2011model}. For that we use the $T_{\max}$ statistic with a moving block bootstrap of length 6 and 1000 bootstrap draws to correct for the autocorrelation of the loss differentials.
\begin{equation}
    T_{\max} = \max_{i \in M} t_{i\cdot}, \quad \text{with} \quad t_{i\cdot} = \frac{\bar{d}_{i\cdot}}{\widehat{\text{SE}}(\bar{d}_{i\cdot})},
\end{equation}
where $\bar{d}_{i\cdot} = |M|^{-1} \sum_{j \in M} \bar{d}_{ij}$ denotes the sample loss of model $i$ relative to the average loss of all remaining models in $M$. The MCS is done on a currated model set of the best valid Models of each Model x regime combination plus the random Walk reference. This is done since on 742 strongly correlated Kombinations the MCS would lose its power to discriminate. We report which Models survive in which countrys in chapter \ref{sec:results}.
...
%------------------------------------------------------------------------
We compared the Models performance on two groups of volatility regimes. First on realised volatility terziles based on the rolling 12 month standard deviation of the \textbf{realized} delta spreads. Here we have the regimes stressed, mid and calm. Second on wether the VIX was higher or less than or equal to 20. here we have the regimes stressed and calm. \\

We evaluate per regime and report coverage and sharpness as well as stress coverage drop ( Coverage(Stressed) -Coverage(calm) ). Here we wanted to test weither complexer Models beat simpler models in stressed regimes. The outcomes can be found in section \ref{sec:results}.
%qunatitative easing gestrichen. diese regime definition wurde entfernt
...
%------------------------------------------------------------------------
Our primary setting for the conformal prediction calibration window was 36 months. To check for robustness of the ranking we also tested with a rolling window of 24 and 36 months as well as an expanding window. This is only relevant for conformal prediction methods methods that employ a pool. We report the spearman roh of the rankings.\\
...
Ireland was the country where the winning methods gained the most Winkler skill, so we ranked the Packages on the dataset excluding Ireland to see if the ranking is stable. We report the spearman roh aof the ranking with and without Ireland.\\
...
For the Neural Network trainings we used three seeds (Seeds 42/43/44). We report the seed span of the skill to distinguish statistical noise from actual effects. \\
...
Since the Burn is set is used for HPO validation set as well as the conformal method calibration pool we comapred the performance on the burnin set with the rest to find out if this distorts the results. \\
...
All aggregates in this paper are a median over all 10 countrys. The median only makes sense if all 10 countrys have a consistent ranking. FOr every country pair (i,j) we calcualte the kendal rank korrealtionskoeffizient $tau_b$ between the skill vectos of the 742 combiantions. We report the mean over all 42 $\binom{10}{2}$ pairs. We use kendal isntead of spearman since tau is more robust to frequent ties and near ties.
\begin{equation}
\tau_b = \frac{C - D}{\sqrt{(C + D + T_x)(C + D + T_y)}}
\end{equation}
where $C$ denotes the number of concordant pairs, $D$ the number of discordant pairs, and $T_x$ and $T_y$ the number of ties present only in $x$ or $y$, respectively. A tau close to 1 means a common ranking and justifys the aggregation. A tau around 0.3-0.4 is a sign of country specific rankings and a tau of 0 means that the aggregation is not interpretable
%  5. LAENDER-KONSISTENZ DER RANKINGS (F7) -- NEU EINZUFUEGEN 2026-08-03.
%     Diese Kennzahl wird in 8.9 BERICHTET, aber bisher nirgends
%     EINGEFUEHRT. Das ist eine Luecke: der Leser trifft dort auf ein
%     Kendall-tau ohne Definition. Hier gehoert die Methode hin.
%     WAS ZU SCHREIBEN IST (2-4 Saetze + Gleichung unten):
%       Motivation: alle Aggregate dieser Arbeit sind Mediane UEBER die
%       zehn Laender (Skalenregel, \cref{sec:eval:skill}). Ein Median ist
%       nur dann eine sinnvolle Zusammenfassung, wenn die Laender
%       ueberhaupt dieselbe Rangordnung erzeugen. Wenn nicht, gibt es
%       nicht EINE Geschichte, sondern zehn.
%       Konstruktion: fuer jedes Laenderpaar (i,j) wird der Kendall-Rang-
%       korrelationskoeffizient tau_b zwischen den Skill-Vektoren der 742
%       Kombinationen gebildet; berichtet wird das Mittel ueber alle
%       45 = C(10,2) Paare. Kendall statt Spearman, weil tau robust
%       gegenueber den vielen nahezu gebundenen Raengen im Mittelfeld ist
%       (und weil die Robustheits-Checks 1-2 bereits Spearman-rho auf
%       einer ANDEREN Achse verwenden -- Verwechslung vermeiden, das im
%       Text kurz sagen).
%     GLEICHUNG (einfuegen, Nummer anpassen):
%       tau_b = (C - D) / sqrt((C + D + T_x)(C + D + T_y)),
%       wobei C die konkordanten, D die diskordanten Paare und T_x, T_y
%       die nur in x bzw. nur in y gebundenen Paare zaehlen.
%     LESEREGEL, die du vorab fixieren solltest (sonst ist die
%       Interpretation in 8.9 post hoc): tau nahe 1 = eine gemeinsame
%       Rangordnung, Aggregation gerechtfertigt; tau um 0.3-0.4 =
%       substanzielle Laenderheterogenitaet, Medianaussagen bleiben
%       gueltig, aber "das beste Verfahren" ist laenderabhaengig;
%       tau nahe 0 = Aggregation nicht interpretierbar.
%
% ZAHLEN AUS:
%   Pool-Fenster: cells.csv, Spalte pool_mode (rolling/expanding/
%       rolling24/rolling48) -- alle vier liegen im SELBEN Lauf, keine
%       separaten Run-Artefakte
%   ex-IE: results/tables/robustness_ex_ie.csv (742 x 16), Spalten
%       median_skill, median_skill_exie, d_skill
%   Seeds: ranking_all.csv, Spalte seed_span
%   Burn-in: cells.csv, Spalten cov_first36 und cov_rest
%   Laender-Konsistenz (F7): berechnet aus cells.csv, pool=rolling,
%       Pivot Kombination x Land auf der Spalte skill
%       -> mittleres tau 0.346 | Median 0.344 | Min 0.073 | Max 0.650
%
% ZITATE: kendall1938 (tau), benjamini1995fdr. IE-Verzerrung ggf. mit
%   Quelle belegen (siehe TODO unten -- offener Punkt).
%   !! kendall1938 fehlt noch in references.bib -- eintragen.

% TODO Prosa 1-5 als kompakte Liste/Absatz. Jeder Check = 1 Satz Motivation
%   + 1 Satz Kennzahl.

% [arXiv-Kuerzung]: 1 Absatz mit 5 Stichpunkten inline ("Pool-Fenster 24/36/48
%   + expanding, ex-IE, Seed-Spannen, Burn-in-Coverage, Laender-Kendall").
