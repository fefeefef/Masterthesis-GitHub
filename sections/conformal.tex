...
$y_t$ be the realised spread level. The conformity score is the CQR score of
...
We cannot calculate a score for the Greek spread in 2015-07/08 since there is no data available. The online methods skip these months and the pool methods never see them. These months lie within the burn-in set; the test set is complete for all countries, as already discussed in \cref{sec:data}.
...
\begin{equation}
  ...
  \label{eq:m-aci}
\end{equation}
initialised at $\alpha_{115} = \alphamis$ with $\gamma = 0.005$.

The best learning rate $\gamma$ is heavily data-dependent, so \texttt{agaci}
\parencite{zaffran2022agaci} turns the choice into an online aggregation
problem over $K$ experts, each running ACI with a different $\gamma$ from the
paper's grid. This method aggregates the predictive bounds rather than the thresholds, so it briefly steps outside our standard score interface. The aggregation uses the BOA algorithm \parencite{wintenberger2017boa} with adaptive learning rates. The lower and upper bounds are aggregated separately, using a symmetric-tail reading of the pinball loss at $\alphamis/2$ for the lower bound and at $1-\alphamis/2$ for
the upper bound, even though our underlying score is one-sided. Aggregating
online means the $\gamma$-sensitivity is handled by the algorithm itself, not
by us tuning it by hand.
...
also propose sampling an expert according to the weights. We didn't do that. Our
...
\texttt{spci} \parencite{xu2023spci} reverses the usual conformal logic.
Instead of taking an empirical quantile of past scores, it predicts the
conditional $(1-\alphamis)$-quantile of the next score, using the most recent
$w=12$ scores as features. We estimate this with a Quantile Random Forest
\parencite{meinshausen2006qrf}, implemented through the leaf weights of a
standard random forest (100 trees, minimum leaf size 2, seed 42). By
exploiting the temporal dependence and volatility clustering of the scores
directly, it is the conceptual opposite of the exchangeability assumption. If
the calibration pool has fewer than $w+10=22$ scores (mainly concerns the 24-month runs) the method falls back to a standard empirical pool
quantile. The original paper also optimizes an asymmetry
parameter $\beta$ to find the tightest interval endpoints, but that degree of
freedom does not exist for our one-sided CQR score, since the interval is
fully determined by the $(1-\alphamis)$-quantile alone. We omit the
$\beta$-search.
...
\subsubsection{Group (e): online control and online convex optimisation}
\label{sec:conformal:online}

What sets the methods in this last category apart from the others is that they keep no historical pool of scores. Instead, the threshold $Q_t$ is an internal state, initialized at zero and updated every month from the instantaneous coverage error. They give no finite-sample exchangeability guarantee, only long-run
asymptotic coverage or regret bounds. All free step sizes are scaled
dynamically in units of $\bar{S}$.

The PID family takes a control-theoretic approach \parencite{angelopoulos2023pid}.
It treats the coverage error ($\mathrm{err}_t - \alphamis$) as the feedback
...
integrator ($C_{\text{sat}} = 5$) to counter sustained miscoverage and drain
the accumulated error. A naming note: our \texttt{pid} is the PI controller
from the original paper, without the derivative term. \texttt{pid\_local}
replaces the global integrator with a rolling 12-month window. It forgets
old errors faster and stays robust when the regime changes.
...
on the realised score history $\{E_s\}_{s<t}$ with mean $\mu$ and clipped
...
descent directly on the pinball subgradient. "Scale-free" means the update
...
accumulate the clipped, normalised instantaneous regrets
...
step as \texttt{sfogd}, but with a step size that decays deterministically.
With exponent $-1/2-\epsilon$ ($\epsilon=0.05$) the step sequence is
square-summable. The tracker settles on a stable threshold instead of
oscillating around the target. It is a deliberate counterweight to
reactive methods like ACI or PID: less adaptivity, in exchange for long-term
stability.
...
\begin{table}[htbp]
  \centering\small
  \caption{The 14 conformal prediction methods applied to every stream, grouped
  by increasing adaptivity (\cref{sec:conformal:baselines}--\cref{sec:conformal:online}).
  All parameters are fixed ex ante (\cref{sec:conformal:params}); $\bar S$ denotes
  the burn-in score scale.}
...
data. Every value comes from one of two ex-ante sources. Either the exact
value from the foundational paper or a standard default expressed in units of $\bar{S}$ (\cref{eq:burnin-scale}). This
constraint matters since tuning the conformal layer would make Research Question 3
circular, because we would then be comparing hyperparameter tuning budgets
instead of the methods themselves. $\bar{S}$ is derived only from the pre-test
burn-in months. So this scale adaptation is free of data leakage. We also did
no grid-search for the ACI learning rate $\gamma$. We let \texttt{agaci}
and \texttt{dtaci} determine the grid online. This is the more honest
approach, but it means we cannot report a single "best" static $\gamma$ for
our dataset. \cref{tab:cp_params} reproduces the parameter dictionary from our
execution environment verbatim.
...
We construct two additional prediction streams purely computationally from
saved model artifacts. No extra training is involved. They serve as the
control arms for Research Questions (1,2) and go through the same 14
calibration methods as every other stream.

The constant-width twin takes the model's median point forecast and
adds or subtracts the empirical 10th and 90th percentiles of that refit
block's in-sample median residuals. It therefore shares the exact same point
forecast as its parent model, but keeps a strictly constant interval width for
the whole refit block. This construction addresses Research Question 2. It
isolates the value of adaptive width control, since the point forecasts are
identical and any performance difference must come from the dynamic,
conditional uncertainty estimate. As noted in \cref{sec:models}, this only
works for the trainable machine learning models (\texttt{xlstm}, \texttt{lstm},
\texttt{lgbm}), since zero-shot foundation models have no in-sample residuals.
...
The Global+Individual (G+I) stream shifts the global G model's quantiles in
parallel, using the rolling median of that country's last 36 realized
out-of-sample median residuals. For Research Question 1, G+I is the cheapest
possible country-specific adaptation. It is a naive intercept shift with no extra
fine-tuning. The contrast between the globally fine-tuned GF regime and G+I
will show whether fine-tuning actually captures cross-country dynamics, or
...
to 36 during burn-in and stays fully populated at 36 from the first test month
...
out-of-sample. This construct only exists for trainable models with a global
regime.
...
All 14 conformal implementations follow their foundational papers, but our
monthly CQR score setup needs a few technical conventions and deliberate
deviations. We list them here for transparency.

\begin{enumerate}
  \item In \texttt{pid\_tan}, the derivative (D) term is a purely additive
  term on the effective threshold $Q^{\text{eff}}_t$. This differs from the
  original composition, where the P+I controller regulates the scorecaster's
  residuals. Our control loop never "sees" the scorecaster's forecast in its
  ...
  raw interval, never narrow it. Its conservative coverage is an artifact
  of its domain definition, not a finding.

  \item For the \texttt{aci} family, when a long violation streak pushes
  $\alpha_t$ below $1/(n+1)$, we substitute the maximum observed score in the
  calibration pool for the theoretically infinite interval. This is a
  necessary finite-pool approximation, and makes the method mildly anti-conservative at extreme miscoverage levels.

  \item For \texttt{dtaci}, we emit the deterministic weighted mean of the
  expert thresholds instead of sampling an expert stochastically. This is done to avoid adding unnecessary sampling variance to our seed-aggregated evaluations.
  ...
  \item In rare cases, driven by extreme $\alpha_t$ values in ACI, the calibrated interval inverts (lower bound above upper bound). We repair
  this by swapping the edges and log the event with a \texttt{crossed\_cal}
  flag.
...
The conformal pipeline ends in a single tidy table, \texttt{calibrated\_all.parquet},
logging country, month, model, regime, seed, CP method, pool mode, target
...
evaluation chapter (\cref{sec:evaluation}), which keeps a clean cut between
