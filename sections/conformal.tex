...
% SCHREIBE (5-7 Sätze).
%  - Ausgangsproblem: die beste Lernrate gamma ist stark datenabhängig.
%  - \texttt{agaci} \parencite{zaffran2022agaci} macht daraus ein
%    Online-Aggregationsproblem über K Experten; jeder Experte fährt ACI
%    mit einem anderen gamma aus dem Gitter des Papers.
%  - PFLICHT: diese Methode aggregiert die PROGNOSEGRENZEN, nicht die
%    Thresholds. Sie ist damit die einzige Ausnahme von eurem
%    Standard-Score-Interface. Das muss dastehen, sonst wirkt eure
%    Schnittstellenbeschreibung inkonsistent.
%  - Aggregation über BOA \parencite{wintenberger2017boa} mit adaptiven
%    Lernraten.
%  - Untere und obere Grenze werden getrennt aggregiert, mit
%    symmetrischer Tail-Lesart des Pinball-Loss bei alpha/2 bzw.
%    1-alpha/2 — obwohl euer Score einseitig ist. Auch das ist eine
%    bewusste Abweichung.
%  - Fazit: weil die Aggregation online passiert, erledigt der
%    Algorithmus die gamma-Sensitivität; ihr müsst nichts von Hand tunen.

The best learning rate $\gamma$ is heavily time dependant. \texttt{agaci} \parencite{zaffran2022agaci} turns this into an aggregationproblem over k experts where every expert uses a different $\gamma$ from the papers grid. This method aggregates the forcasted bounds not the thresholds and therefore is an exception from our standard score interface. The aggregation worts via BOA \parencite{wintenberger2017boa} with adaptive learning rate. Lower and upper bounds are aggregated independantly with symmetric tail reading of the pinball loss at $\frac{\alpha}{2}$ or $1-\frac{\alpha}{2}$ eventhough our scores are one sided. Since the aggregations happens online the algorithmus handles the $\gamma$ sensitivity.
...
% SCHREIBE (6-8 Sätze).
%  - Der Grundgedanke: statt ein empirisches Quantil vergangener Scores
%    zu nehmen, PROGNOSTIZIERT spci \parencite{xu2023spci} das bedingte
%    (1-alpha)-Quantil des nächsten Scores aus den letzten w=12 Scores.
%    -> Original: "reverses the usual conformal logic" bzw. "turns ...
%       around" (Tell). Beschreib schlicht den Unterschied: die anderen
%       Methoden schauen zurück, diese schätzt nach vorn.
%  - Schätzer: Quantile Random Forest \parencite{meinshausen2006qrf},
%    umgesetzt über die Blattgewichte eines Standard-Random-Forest
%    (100 Bäume, minimale Blattgröße 2, Seed 42).
%  - PFLICHT: die Methode nutzt gezielt die zeitliche Abhängigkeit und
%    das Volatilitätsclustering der Scores — also genau das, was die
%    Austauschbarkeitsannahme ausschließt.
%    -> Original: "conceptual opposite of the exchangeability
%       assumption" (Tell). Sag es als Beobachtung: deshalb passt sie zu
%       Monatsdaten mit Regimewechseln.
%  - Fallback: weniger als w+10 = 22 Scores im Pool (betrifft vor allem
%    die 24-Monats-Läufe) -> zurück auf das empirische Pool-Quantil.
%  - Abweichung: das Originalpaper optimiert zusätzlich einen
%    Asymmetrieparameter beta für die engsten Intervallgrenzen. Bei
%    eurem einseitigen CQR-Score existiert dieser Freiheitsgrad nicht,
%    das Intervall ist durch das (1-alpha)-Quantil allein bestimmt.
%    Deshalb entfällt die beta-Suche.
%    -> Damit ist Item 6 der Implementation notes (3.11) inhaltlich
%       erledigt; schreib beide in einem Zug.

Instead of taking an empiric quantile from past scores spci \parencite{xu2023spci} predicts the dependant (1-alpha)-Quantile of the next score from the last w=12 scores. Quantile Random Forest \parencite{meinshausen2006qrf} implemented via the leafweights of a standard random forest (100 trees, min leafsize 2, Seed 42). The method uses the time dependance and volatility clustering of the scores so exactly what the exchangability assumtion excludes. If there are less than w+10 = 22 scores in the pool (applys in the 24 month runs) the method falls abck to the standard epirical pool quantiles. The original paper optimzes an asymmetrieparameter $\beta$ for the narrowest intervall bounds. In our onesided CQR score this degree of freedom doesent exist so the itnervall is determined only by the (1-alpha)-Quantile. Therefore we dont ahve to look for a $\beta$
...
% SCHREIBE (4-5 Sätze).
%  - Was diese letzte Gruppe unterscheidet: sie hält KEINEN historischen
%    Score-Pool.
%  - Stattdessen ist der Threshold Q_t ein interner Zustand, bei null
%    initialisiert, monatlich aus dem momentanen Coverage-Fehler
%    aktualisiert.
%  - PFLICHT: diese Methoden geben KEINE finite-sample-Garantie unter
%    Austauschbarkeit, nur asymptotische Langfrist-Coverage oder
%    Regret-Schranken. Das ist der eigentliche Unterschied zu den
%    Pool-Methoden und gehört klar gesagt.
%  - Alle freien Schrittweiten sind in Einheiten von S-quer skaliert
%    (\cref{eq:burnin-scale}).

This last group is different in the way that it doesent need a historical score pool, insted the threshold $Q_t$ is initialiced at zero and and monthly updated based on the current coverage error. These methods give no finite sample guarantee und the exchangability assumtion, only asymtotical long term coverage or rregret bounds. All step sizes are scaled in units of $\bar{S}$ (\cref{eq:burnin-scale}).

% SCHREIBE (6-8 Sätze).
%  - Herkunft: Regelungstechnik \parencite{angelopoulos2023pid}.
%  - Das Rückkopplungssignal ist der Coverage-Fehler (err_t - alpha).
%  - P-Term: einfacher Quantil-Tracking-Schritt mit Verstärkung
%    eta = 0.1 * S-quer.
%  - I-Term: saturierender Tangens-Integrator (C_sat = 5) gegen
%    anhaltende Miscoverage; er baut den akkumulierten Fehler ab.
%  - PFLICHT (Namenshinweis, sonst missversteht der Leser die Tabelle):
%    euer \texttt{pid} ist der PI-Regler des Originalpapers, OHNE
%    D-Term.
%  - \texttt{pid\_local}: globaler Integrator wird durch ein rollierendes
%    12-Monats-Fenster ersetzt -> alte Fehler werden schneller vergessen,
%    robuster bei Regimewechseln.
%  - \texttt{pid\_tan}: bringt den D-Term über einen autoregressiven
%    Scorecaster zurück.
%  - Querverweis auf die Implementation notes, wo die Abweichung bei
%    pid_tan steht (die hast du schon geschrieben).

The PID family comes from control theory \parencite{angelopoulos2023pid}. The feedbacksignal is the coverage error $(err_t - alpha)$. The p term is a simplke quantile step tracker with a scaler $\eta = 0.1 * \bar{S}$. The i term is a saturated tangens integrator $(C_{sat} = 5)$ to work agaoinst ongoing miss coverage. He dimishises the accumulated error. Our \texttt{pid} is the PI controler from the original paper without D term. In \texttt{pid\_local} the global itnegrator is substituted by a rolling 12 month window which leads to faster forgetting of previous errors and therefore possibly more robistnes against regime change. ( schreibe den querverweis hier)
...
% SCHREIBE (3-4 Sätze).
%  - \texttt{sfogd} \parencite{bhatnagar2023sfogd} läuft als scale-free
%    Online-Gradientenabstieg direkt auf dem Pinball-Subgradienten.
%  - Was "scale-free" heißt: der Update-Schritt wird durch die Wurzel der
%    akkumulierten quadrierten Gradienten normiert.
%  - FOLGE, und das ist der Verkaufspunkt der Methode: es gibt keine
%    Lernrate zu kalibrieren, nur einen Basisradius D = S-quer.
%  - Die Korrekturschritte sind anfangs groß und schrumpfen automatisch.
%    -> Original: "This gives large corrective steps early on that get
%       smaller automatically over time." (Tell aus deiner alten Liste).
%       Kürzer und in deinem Ton sagen.

\texttt{sfogd} \parencite{bhatnagar2023sfogd} is a scale free online gradient decent on the pinball sub gradient. scale free here meant that the update step is normed by the root of the accumulated squared gradient. That makes the search for the optimal learning rate obsolete, we only need a base radius D = S-quer. The correction step are big in the beginning an shripk over time automatically.
...
% SCHREIBE (3-4 Sätze).
%  - \texttt{decay\_ocp} \parencite{angelopoulos2024decay} nutzt denselben
%    Tracking-Schritt wie sfogd, aber mit deterministisch abfallender
%    Schrittweite.
%  - Exponent -1/2 - epsilon (epsilon = 0.05) -> die Schrittfolge ist
%    quadratsummierbar.
%  - FOLGE: der Tracker läuft auf einen stabilen Threshold zu, statt um
%    das Ziel zu oszillieren.
%  - WOZU im Vergleich: als Gegenstück zu den reaktiven Methoden wie ACI
%    oder PID. Es tauscht Adaptivität gegen Langfriststabilität.
%    -> Original: "a deliberate counterweight to reactive methods ...
%       less adaptivity, in exchange for long-term stability" (Tell).
%       Sag es als deine Auswahlbegründung: du wolltest beide Enden des
%       Spektrums im Feld haben.

\texttt{decay\_ocp} \parencite{angelopoulos2024decay} uses the same tracking step as sfogd but with deterministic shriking step size. The exponent is $-1/2 - \epsilon\ (\epsilon = 0.05)$ and therefore the series is square summable. This lets the tracker converge towars a stable threshold instead of oszillating around the target. In oposition to the reactive methods like aci or pid this method trades adaptivity against longrun stability
...
%------------------------------------------------------------------------
% SCHREIBE (2-3 Sätze).
%  - Zwei zusätzliche Prognoseströme, rein rechnerisch aus den
%    gespeicherten Modellartefakten gebaut — ohne zusätzliches Training.
%  - WOZU: sie sind die Kontrollgruppen für RQ1 und RQ2.
%  - Sie durchlaufen dieselben 14 Kalibrierungsmethoden wie jeder andere
%    Strom (Gleichbehandlung).

We construct two additional prediction streames from the saved model artifacts, without training. These serve as a basis for reasearch question one and two. The go though the same 14 calibration methods as every other stream.

% SCHREIBE (5-7 Sätze).
%  - Konstruktion: der Twin nimmt die Median-Punktprognose des Modells und
%    addiert/subtrahiert die empirischen 10.- und 90.-Perzentile der
%    In-Sample-Median-Residuen desselben Refit-Blocks.
%  - Ergebnis: exakt dieselbe Punktprognose wie das Elternmodell, aber
%    konstante Intervallbreite über den ganzen Refit-Block.
%  - PFLICHT, das ist der Zweck: Kontrollgruppe für RQ2. Weil die
%    Punktprognosen identisch sind, MUSS jeder Leistungsunterschied aus
%    der dynamischen, bedingten Unsicherheitsschätzung kommen. Ohne
%    diesen Satz ist der Twin nur eine Variante.
%  - Einschränkung: funktioniert nur für die trainierbaren Modelle
%    (xlstm, lstm, lgbm), weil die Zero-Shot-Modelle keine
%    In-Sample-Residuen haben (\cref{sec:models}).
%  - Wie jeder andere Strom durchlaufen die Twins die monotone
%    Rearrangierung und das Validity-Flagging.

The cosntant width twin takes the median point forecast of the model and adds/substacts the empirical 10/90 percentileof the in sample median residuals of the same refit block. This gives us exactly the same pointforecast as the parentmodel but constant intervall width over the whole refit block. This is done to build a controllgroup for resear question 2, since the pointforecasts are identical the difference in performance must stem from dynamic uncertainty quantification. This is only done for the trainable models (xlstm, lstm, lgbm) since zero shot models dont have any in sampe residuals (\cref{sec:models}). As all other streams the constant width twins go through the monotone rearangement and validity flagging. 
...
% SCHREIBE (7-9 Sätze).
%  - Konstruktion: der G+I-Strom verschiebt die Quantile des globalen
%    G-Modells parallel um den rollierenden Median der letzten 36
%    realisierten Out-of-Sample-Median-Residuen des Landes.
%  - Es ist ein naiver Achsenabschnitts-Shift, ohne Fine-Tuning — also
%    die billigste denkbare länderspezifische Anpassung.
%  - PFLICHT, der Zweck für RQ1: der Vergleich GF gegen G+I zeigt, ob
%    Fine-Tuning tatsächlich länderübergreifende Dynamik lernt oder nur
%    eine grobe Niveaukorrektur leistet. Das ist die eigentliche Frage.
%  - Leakage-Schutz: der in Monat t angewendete Shift nutzt nur Residuen
%    von VOR t.
%  - Das rollierende Fenster wächst während des Burn-in von 1 auf 36 und
%    ist ab dem ersten Testmonat voll besetzt.
%  - Diagnostik: In-Sample- und Out-of-Sample-Median-Residuen werden je
%    Refit-Block protokolliert, um zu prüfen, ob der im Training
%    sichtbare Bias out-of-sample überlebt.
%  - Geltungsbereich: nur für trainierbare Modelle mit globalem Regime.

The G+I stream moves the quantiles of the global G Model parallel by the rolling median of the last 36 realised out of sample median residuals of the country. This is a naive shift of the axis sction without fine tuning, so the cheapest possible country specific adaption. This is done to give clear contrast for research question 1 where we want to find out weither fine tuning actually learns dynamics across countrys or only corrects for a nivau. The in month t applyed shift only usees residuals before t. The rolling window grows durcing the burnin phase from 1 to 36 and is fll strating from the first test month. In sample and out of sample median residuals are saved per refit block to test weither the bias in training is measurable out of sample. All this is done only for the trainable models wit global regime. 
...
%------------------------------------------------------------------------
% SCHREIBE: A14 — Einleitung (2-3 Sätze).
%   - Alle 14 Implementierungen folgen ihren Originalpapern.
%   - ABER: das monatliche CQR-Setup braucht ein paar technische
%     Konventionen und bewusste Abweichungen.
%   - Warum das hier steht: Transparenz. Sag es als deine Entscheidung
%     ("we list them" -> warum listest DU sie? weil ein Referee sonst zu
%     Recht fragt, ob die Implementierung dem Paper entspricht).
%   - Der Unterschied zwischen "technische Konvention" (erzwungen durch
%     Monatsdaten) und "bewusste Abweichung" (deine Wahl) lohnt einen
%     Halbsatz — die Liste enthält beides.

All 14 implementations follow the paper they originate from. Non the less the monthly CQR setup needs some technical conventions and some deliberate deviations. We list them here for transperacy.

% SCHREIBE: A15 — pid_tan (3-4 Sätze).
%   - WAS wir anders machen: der D-Term wird direkt auf die effektive
%     Schwelle Q^eff_t addiert.
%   - WAS im Original steht: dort regelt der P+I-Controller die
%     Residuen des Scorecasters.
%   - DIE FOLGE, und das ist der Punkt des Items: in unserer Version
%     sieht der Regelkreis die Scorecaster-Prognose nie in seinem
%     Fehlersignal. Der D-Term kann den Integrator also nicht
%     mechanisch entlasten.
%   - PFLICHT: die Folge muss dastehen, nicht nur der Unterschied.
%     Sonst liest es sich wie ein Implementierungsdetail statt wie das,
%     was es ist — eine Verhaltensänderung des Reglers.
%   - Optional, wenn du es belegen willst: pid_tan hat die höchste
%     Kreuzungsrate aller Methoden (2.554 %, cells.csv) und den
%     schlechtesten Median-Skill. Das passt zu dieser Abweichung.
%     Nur schreiben, wenn du die Zahlen prüfst.

\begin{enumerate}
  \item In \texttt{pid\_tan} the D thermis added directly to the effective threshold $Q^{\text{eff}}_t$. In the original method in the paper the residuals of the scorecaster are controlled by the P+I controller. in our version the scorecaster forecast is nerver seen in an error loop. It follows that the D therm relieve the Integrator.

% SCHREIBE: A16 — saocp (3 Sätze).
%   - Die Referenzimplementierung erzwingt nichtnegative Radien.
%   - Auf unseren VORZEICHENBEHAFTETEN CQR-Scores heißt das: der
%     Algorithmus kann das rohe Intervall nur verbreitern, nie verengen.
%   - PFLICHT, das ist die wichtigste Aussage des ganzen Unterabschnitts:
%     die konservative Coverage von saocp folgt damit aus der
%     Domänendefinition. Sie ist KEIN Befund. Wer das nicht sagt,
%     berichtet ein Artefakt als Ergebnis.
%     -> Diesen Satz musst du in der Defensio vertreten können. Formulier
%        ihn so, dass du ihn im Schlaf erklären kannst.
%   - Schöner Nebenbeleg, falls du ihn willst: saocp und native kreuzen
%     exakt nie (0.000 %). Bei native per Konstruktion, bei saocp wegen
%     genau dieser Nichtnegativität. Das bestätigt die Behauptung
%     empirisch.

   \item In the \texttt{saocp} reference non negative radii are forced. Since we work on signed CQR scores this means that the algorithm cant narrow but only widen the intervall. The conservative bias of \texttt{saocp} follows by definition. 

% SCHREIBE: A17 — aci (3-4 Sätze).
%   - Eine lange Violation-Streak kann alpha_t unter 1/(n+1) drücken.
%   - Dann wäre das Intervall theoretisch unendlich.
%   - Was wir stattdessen tun: das Maximum der beobachteten Scores im
%     Kalibrierungspool.
%   - PFLICHT (steht so auch in \cref{sec:conformal:score}): das ist eine
%     finite-pool approximation und macht die Methode bei extremen
%     Miscoverage-Levels MILD ANTIKONSERVATIV. Beide Wörter müssen
%     fallen — "antikonservativ" heißt, die Intervalle sind eher zu eng,
%     und das ist die Richtung, die einem Prüfer auffällt.
%   - Achte darauf, dass die Formulierung zu der in
%     \cref{sec:conformal:score} passt; dort steht dieselbe Aussage
%     schon einmal. Zwei verschiedene Formulierungen desselben
%     Sachverhalts wirken wie zwei verschiedene Aussagen.
  \item In the \texttt{aci} family a long violation streak can decrease $\alpha_t$ to less than $\frac{1}{n+1}$ and by that lead to infinite intervalls. Insted we decided to cap widdening of the intervall to the maximum score in the calibration pool. This is a finite pool approximation and leads to the method being mildly anti conservative in extreme miscoverage levels as already statet in \cref{sec:conformal:score}. 

% SCHREIBE: A18 — dtaci (2-3 Sätze).
%   - Wir geben den deterministischen gewichteten Mittelwert der
%     Expertenschwellen aus.
%   - Das Originalpaper schlägt stattdessen vor, einen Experten
%     stochastisch nach den Gewichten zu ziehen.
%   - BEGRÜNDUNG, und die ist deine: unsere Auswertung aggregiert
%     ohnehin schon über mehrere Seeds. Zusätzliche Sampling-Varianz
%     würde die methodischen Kontraste nur verwischen.
%   - Das ist eine saubere, verteidigbare Entscheidung — schreib sie als
%     Entscheidung, nicht als Verzicht.

  \item For \texttt{dtaci} we use the deterministicly weighted average of the expert threshold. Instead the original paper proposes to draw expers stochasticly by weights. Our evaluation already aggregates over multiple seeds added sample variance would only weaken the methodic contrasts. 
...
% SCHREIBE (3-4 Sätze).
%  - Die Pipeline endet in einer einzigen Tabelle,
%    \texttt{calibrated\_all.parquet}.
%    -> Original war "ends in a single tidy table" (Tell aus deiner alten
%       Liste). "tidy" weg, es sagt nichts.
%  - Was drinsteht: Land, Monat, Modell, Regime, Seed, CP-Methode,
%    Pool-Modus, Realisierung des Targets, Level-Anker y_{t-1},
%    Punktprognose sowie rohe UND kalibrierte Quantile — je Strom.
%  - PFLICHT: diese Tabelle ist der einzige Input für das
%    Evaluationskapitel (\cref{sec:evaluation}). Deshalb bleiben
%    Kalibrierung und Inferenz sauber getrennt. Das ist eine
%    Reproduzierbarkeitsaussage, sag sie als solche.

The pipeline ends in a single table, \texttt{calibrated\_all.parquet}. It contains country, month, model, regime, seed, cp method, pooling mode, realized target, level anchor $y_{t-1}$, point forecast aswell as raw and calibrated quantiles per stream. This table is the sole input for the evaluation chapter (\cref{sec:evaluation}), therefore calibrationa nd evaluation stay clearly sperated which makes our results reproducable.
