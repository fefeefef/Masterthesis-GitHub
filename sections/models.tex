...
the recurrent cell. It is therefore the ablation that isolates what the xLSTM
adds: any skill gap between the two is attributable to the cell, not to the
pipeline.
...
attention's key--value cache, giving constant memory and linear compute.
\begin{align}
  q_t &= W_q x_t + b_q, &
...
the pretrained models, closing the ladder of models. They are pretrained on large
...
the 17 covariates as past covariates. The within-model contrast of TiRex-2 with and
without covariates on identical test months isolates the effect of the additional
covariates. The covariates enter as past covariates; we have no future-known
features, so TiRex-2's bidirectional future-covariate path is not exercised in our
