...
capital controls during the Greek crisis. This missing window is carried forward and does not lie in the test set. A second limitation of including Greece is the 2012 private-sector
...
good fit, since \textcite[ECB WP 1781]{afonso2015} investigate spread drivers
...
available to us. Because realized data is subject to revisions that inject
...
use the bid-ask spread, but here too no free data was available.
\Cref{tab:features_overview} lists all the features we use.

\begin{table}[htbp]
    \centering
    \small
    \caption{Feature catalogue with identifier, construction, scope and source. Country fundamentals enter as differentials against Germany; global features are identical across countries.}
...
\textcite[ECB WP 1781]{afonso2015} use only the VIX as a global risk factor;
...
\footnote{We aimed to include the \texttt{us\_hy\_oas} = US High Yield Option-Adjusted Spread as an indicator of the global risk tolerance, but without a license it was only available three years into the past.}
...
\textcite{afonso2012sovereign} have shown that the numeric sovereign rating on a
...
\texttt{pc2\_core\_periph} is calculated from the z-standardized country data,
with a 12-month refit on an expanding window of minimum 36 months. The loadings
are anchored so that periphery countries always load positively. The missing value
...
\begin{table}[H]
\centering
\caption{Publication lag by feature. Shift is the whole-month lag we impose in real time, $k=\lceil \text{days}/30\rceil$, on a month-end series.}
...
\textit{Notes.} Lag (d) reports the provider's documented publication delay after the reference period. Shift (m) is applied as a month-end shift of the feature within each country. $^{\dagger}$ marks a monthly-average series whose release delay rounds to a zero-month shift. $^{\ddagger}$ marks a split lag. We lag each input series on its own calendar before combining them, so \texttt{target2\_gdp} enters as $\text{TARGET2}(t{-}2)/\text{GDP}(t{-}4)$ and the interaction terms carry the debt lag only.
\end{minipage}}
\end{table}
