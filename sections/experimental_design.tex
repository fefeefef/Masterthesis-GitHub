...
\end{itemize}
This gives a fair comparison between a local, a global and a specialised model.
...
$z$-standardisation, fitted only on the training window of each refit, so no
...
hyperparameter optimisation. Evaluation takes place on months 151 to 277, that
is 2015-10 to 2026-04, giving 127 test months per country.

\begin{table}[htbp]
    \centering
    \small
    \caption{Refit block structure. Burn-in blocks initialise the conformal
    calibration pool and are not evaluated.}
...
\label{sec:exp:hpo}
The hyperparameter optimisation of all tuned models uses months 1 to 114 as the
...
test-window information enters the hyperparameter selection. Every model with an
information horizon is restricted to a shared maximum of 24 months, so that no
model can use a longer lookback than the others. Wherever a loss had to be
minimised, the selection criterion was the mean pinball loss on the standardised
$\Delta$ scale, averaged across countries, over the burn-in validation months.

\begin{table}[htbp]
  \centering\small
  \caption{Model selection across families. All trainable models share the
  refit-block structure, per-country scaling and the 24-month information cap;
  they differ only as listed here.}
...
\label{sec:exp:seeds}
The three seeds (42, 43, 44) apply to the neural models, xLSTM and LSTM, only.
They measure training stochasticity. We report coverage as the mean $\pm$ sd
across seeds, and downstream we report the span of the median skill across seeds
as a robustness check. Hyperparameters are tuned once, at seed 42
(\cref{sec:exp:hpo}), and then frozen. The seeds differ only in the production
refits. The run loop iterates L, G, GF $\times$ blocks $\times$ seeds. Each seed
is a fully independent refit/predict chain. Its seed label stays on every
prediction row through the conformal and diagnostic stages, and predictions are
never pooled across seeds before the per-chain metrics are computed. The
level-scale metrics (Winkler score, pinball loss, average width, and median MAE)
are computed per country, that is, per regime, seed, and country, so that no
single high-spread country dominates the aggregate. Only the scale-free metrics,
mean coverage rate and mean crossing rate, are averaged across countries. This is
the sole cross-country aggregation. The remaining models, RW, ARIMA, LightGBM,
TiRex, and TiRex-2, are run deterministically with a single seed.



\subsection{Quantile post-processing}
\label{sec:exp:rearrange}
Because the three quantiles are estimated independently, they can cross, which
would leave the conformal layer downstream with ill-defined interval bounds.
Before evaluation we therefore sort the three values per forecast, a monotone
rearrangement \parencite{chernozhukov2010} that guarantees
$\hat{q}_{0.1} \le \hat{q}_{0.5} \le \hat{q}_{0.9}$. The raw crossing incidence,
measured before sorting, is logged as a diagnostic and reported per model. The
step is identical for every model.

\subsection{Conformal prediction layer}
\label{sec:exp:conformal-ref}
The benchmarks emit only raw quantiles; no calibration happens inside them. The
...
calibrated on a rolling out-of-sample pool. Applying the same set of methods to
every model lets us trace differences in interval quality back to the models
rather than to the calibration. The methods, their fixed parameters, and the
calibration-pool variants are described in \cref{sec:conformal}.