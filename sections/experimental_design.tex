...
test-window information enters the hyperparameter selection. Every  trainable model with an information horizon is restricted to a shared maximum of 24 months, so that no model can use a longer lookback than the others. TiRex and TiRex2 use the full expanding level history, which gives an information advantage to these models. Wherever a loss had to be minimised, the selection criterion was the mean pinball loss on the standardised $\Delta$ scale, averaged across countries, over the burn-in validation months.
